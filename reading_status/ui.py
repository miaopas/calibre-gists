...
class InterfacePlugin(InterfaceAction):
    ...
    def update_turn_direction_column(self, book_ids):
        ...
        for book_id in book_ids:
            ...
            read = db.field_for('#read', book_id)
            reading = db.field_for('#reading', book_id)
            # not read -> reading -> finished
            # raise Exception(str(read_mi['#value#'])+ str(type(read_mi['#value#'])))
            if read:
                # if currently is finished 
                read_mi = ''
                reading_mi = ''
            elif reading:
                # if currently is reading
                read_mi = True
                reading_mi = ''
            else:
                # If currently is unread
                read_mi = ''
                reading_mi = '正在阅读'
            
    
            # mi.set_user_metadata('#read', read_mi)
            # mi.set_user_metadata('#reading', reading_mi)
            # db.set_metadata(book_id, mi)
            db.set_field('#read', {book_id:read_mi})
            db.set_field('#reading', {book_id:reading_mi})
